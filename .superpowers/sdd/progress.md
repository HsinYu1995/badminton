...
Google sign-in plan: complete pending one outstanding manual step (see below).
...
- **Not yet confirmed:** the actual end-to-end click-through with this fix in place (user was retrying at time of writing). `adb reverse` is a per-adb-session, per-emulator-boot setting - it does not persist across emulator restarts and needs to be re-run each time the emulator is freshly booted (not on every `expo start`, just after an emulator cold start). Worth a one-line reminder in local dev docs if this becomes a recurring friction point.
